...
def check_capital_portfolio_risk(portfolio_snapshot: dict[str, Any]) -> tuple[bool, str]:
    ...
    # blocking new trades at just ~51% exposure in production (real
    # portfolio: 23 positions, 51% exposure, 45 >= 40 threshold).
    # Position-count bands above are UNCHANGED — this is a capital-
    # utilization fix only, not a position-limit or risk-management
    # change.
    if exposure >= 0.95:
        portfolio_risk += 40.0
    elif exposure >= 0.85:
